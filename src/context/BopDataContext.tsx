...
export function BopProvider({ children }: { children: React.ReactNode }) {
    ...
    const addBop = (bop: Omit<Bop, 'id'>) => {
        ...
        addLog('Created BOP Record', `Business Name: ${newBop['BUSINESS NAME & ADD']}`);
    ...
    const updateBop = (updatedBop: Bop) => {
        ...
        addLog('Updated BOP Record', `Business Name: ${updatedBop['BUSINESS NAME & ADD']}`);
    };
    ...
    const deleteBop = (id: string) => {
        ...
        setAndPersistBopData(updatedData, headers);
        if (bopToDelete) {
            addLog('Deleted BOP Record', `Business Name: ${bopToDelete['BUSINESS NAME & ADD']}`);
        }
    };
