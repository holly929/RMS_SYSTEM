...
const DemandNoticeSheet = React.forwardRef<HTMLDivElement, { bops: Bop[], settings: { general: GeneralSettings, appearance: AppearanceSettings } }>(({ bops, settings }, ref) => {
    return (
        <div ref={ref}>
                ...
                {bops.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-[calc(100vh-10rem)] text-center">
                        ...
                        <p className="text-muted-foreground mt-2">
                            It seems no BOP records were selected. Please go back to the BOP Billing page and select some records to print.
                        </p>
                    </div>
                )}
            </div>
...
export default function BopDemandNoticePrintPage() {
  ...
  const recordBills = async () => {
    ...
    const newBills: Omit<Bill, 'id'>[] = renderedBops.map(b => {
        const totalAmountDue = Number(b['AMOUNT']) || 0;
    ...
    if (newBills.length > 0) {
        ...
        if (success) {
          toast({
              ...
              description: `${newBills.length} BOP demand notices have been recorded in the bill history.`,
          });
        }
    }
  };
      ...
      <div className="flex flex-col min-h-screen bg-background">
        ...
        <main className="flex-grow flex flex-col items-center justify-center text-center p-4">
          ...
          <p className="text-muted-foreground mt-2 max-w-md">
            We are preparing {allBops.length} demand notices for printing. This may take a moment.
          </p>
          <div className="w-full max-w-md mt-6">
            <Progress value={allBops.length > 0 ? (progress / allBops.length) * 100 : 0} />
            <p className="text-sm text-muted-foreground mt-2">
              {progress} / {allBops.length} demand notices ready
            </p>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <header className="no-print bg-card border-b p-4 flex flex-col sm:flex-row items-center justify-between sticky top-0 z-10 gap-4">
        <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            <h1 className="text-lg sm:text-xl font-semibold">
                Demand Notice Preview ({allBops.length} {allBops.length === 1 ? 'Notice' : 'Notices'})
            </h1>
        </div>
      ...
      <div className="invisible h-0 overflow-hidden print:visible print:h-auto print:overflow-visible">
        <DemandNoticeSheet ref={componentRef} bops={renderedBops} settings={settings} />
      </div>
    </div>
  );
}
